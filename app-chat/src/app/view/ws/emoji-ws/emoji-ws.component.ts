...
import { EmojiWsService } from 'src/app/service/ws/emoji/emoji-ws.service';

@Component({
  selector: 'app-emoji-ws',
  templateUrl: './emoji-ws.component.html',
  styleUrls: ['./emoji-ws.component.scss']
})
export class EmojiWsComponent implements OnInit {
  ...
  constructor(
    public emoji_service: EmojiWsService
  ) { }
  ...
  public selectIconNonHistory(item: EmojiObjectWS) {
    this.icon.emit(item);
    this.emoji_service.updateHistory(item);
  }

  public selectIconHistory(item: EmojiObjectWS) {
    this.icon.emit(item);
  }
