...
export class StickerWsService {
  ...
  accessUseSticker() {
    return this.db.database.ref('su_dung_nhan_dan_ws');
  }
    ...
    this.accessStickers().once('value', (sticker) => {
      ...
      sticker.forEach(element => {
          if(loop < 5 && element.val().ton_tai == 0) {
            this.db.database.ref('su_dung_nhan_dan_ws').child(id).child(element.key).set({
  ...
  accessAccessHistorySticker() {
    return this.db.database.ref('lich_su_su_dung_nhan_dan_ws');
  }
}
