...
import { ObjectChatWS } from 'src/app/models/ws/chat-page/chat-page-chat-page/header/object_chat_ws';
import { MyNameWsService } from '../../../my-name/my-name-ws.service';
import { ChatPageChatPageContentWsService } from '../chat-page-chat-page-content/chat-page-chat-page-content-ws.service';


@Injectable({
  providedIn: 'root'
})
export class MessengerHeaderWsService {

  // Đối tượng đang chat 
  public object_chat: ObjectChatWS;
  ...
  constructor(
    ...
    this.object_chat = new ObjectChatWS();
  ...
  public roiKhoiNhom(mctc: string) {
    ...
    this.db.object("/thanh_vien_cuoc_tro_chuyen_ws/" + mctc + "/" + ma_tai_khoan).update({ roi_chua: "roi", ngay_roi_di: currentTime });
  }
  ...
  public dienThanhVien(object: Object) {
    let array: ObjectChatThanhVienWS[] = [];
    if (object != null) {
      Object.entries(object).forEach(([ma_thanh_vien, data_thanh_vien]) => {
        let objectChatThanhVien = new ObjectChatThanhVienWS();
    ...
    Object.entries(object).forEach(([ma_thanh_vien, data_thanh_vien]) => {
      for (let i = 0; i < this.object_chat.thanh_vien.length; i++) {
        if (this.object_chat.thanh_vien[i].ma_tai_khoan == ma_thanh_vien) {
          ...
          this.db.database.ref('cai_dat_ws').child(ma_thanh_vien).on('value', set => {
