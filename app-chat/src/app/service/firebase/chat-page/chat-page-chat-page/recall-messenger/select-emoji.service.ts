...
export class SelectEmojiService {
  ...
  public tim: CamXucTinNhan[];
  public cuoi: CamXucTinNhan[];
  public wow: CamXucTinNhan[];
  public buon: CamXucTinNhan[];
  public gian: CamXucTinNhan[];
  public like: CamXucTinNhan[];
  public dislike: CamXucTinNhan[];
  public count: number;
  public now: CamXucTinNhan[];
  public mtn: string;
  public mctc: string;
  ...
  public openDetail(array: CamXucTinNhan[], mtn: string, mctc: string) {
  ...
  public getData() {
    this.isLayThongTin = this.db.object("/tai_khoan").snapshotChanges().subscribe(data => {
  ...
  public go(item: CamXucTinNhan, i: number) {
    if (item.is_ban_than) {
      let mtk = JSON.parse(localStorage.getItem("ma_tai_khoan_dn"));
      this.db.object("/chi_tiet_cuoc_tro_chuyen/" + this.mctc + "/" + this.mtn + "/cam_xuc_tin_nhan/" +mtk).remove();
      this.is_show_detail =false;
    }
  }
}
