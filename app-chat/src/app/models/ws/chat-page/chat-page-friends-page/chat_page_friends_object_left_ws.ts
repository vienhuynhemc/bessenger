import { ChatPageCuocTroChuyenWS } from "./chat_page_cuoc_tro_chuyen_ws";
import { ChatPageObjectImgWS } from "./chat_page_object_img_ws";
...
export class ChatPageFriendsObjectLeftWS {

    cuoc_tro_truyen: ChatPageCuocTroChuyenWS;
    thong_tin_thanh_vien: ChatPageObjectTinNhanFriendWS[];
    // Trạng thái online
    ...
    public noiDungCuoiCung: ChatPageObjectTenWS = new ChatPageObjectTenWS();
    ...
    public name: ChatPageObjectTenWS = new ChatPageObjectTenWS();
    // Có phải bản thân đang chiếm vị trí cuối cùng
    public itMe: boolean;
    // Danh sách những người đã xem
    public imgUserSeened: ChatPageObjectImgWS[] = [];
    ...
    public getName() {
        let result: ChatPageObjectTenWS = new ChatPageObjectTenWS();
        let ma_tai_khoan = JSON.parse(localStorage.getItem("ma_tai_khoan_dn_ws"));
        if (this.cuoc_tro_truyen.loai_cuoc_tro_truyen == "nhom") {
    ...
    public handleNoiDung(result: ChatPageObjectTenWS) {
    ...
    public getImgUserSeened() {
        let result: ChatPageObjectImgWS[] = [];
        if (this.cuoc_tro_truyen.tin_nhan != null) {
            let ma_tai_khoan = JSON.parse(localStorage.getItem("ma_tai_khoan_dn_ws"));
            if (this.cuoc_tro_truyen.tin_nhan.length > this.viTriCuoiCung) {
                for (let i = 0; i < this.cuoc_tro_truyen.tin_nhan[this.viTriCuoiCung].tinh_trang_xem.length; i++) {
                    if (this.cuoc_tro_truyen.tin_nhan[this.viTriCuoiCung].tinh_trang_xem[i].xem_chua == "roi") {
                        if (this.cuoc_tro_truyen.tin_nhan[this.viTriCuoiCung].tinh_trang_xem[i].ma_tai_khoan != ma_tai_khoan) {
                            if (!this.cuoc_tro_truyen.tin_nhan[this.viTriCuoiCung].tinh_trang_xem[i].is_roi_chua) {
                                if (result.length < 3) {
                                    let object = new ChatPageObjectImgWS();
